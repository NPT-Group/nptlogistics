...
export default function GuidedChatbot() {
  ...
  return (
    <div className="fixed bottom-5 right-5 z-50">
      ...
      <div
        ref={panelRef}
        className={["w-[360px] max-w-[92vw] overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-2xl", open ? "block" : "hidden"].join(" ")}
        role="dialog"
        aria-label="Chatbot"
      >
        {/* Header */}
        <div className="flex items-center justify-between border-b border-gray-200 px-4 py-3">
          ...
          <button type="button" onClick={() => setOpen(false)} className="rounded-md p-2 text-gray-600 hover:bg-gray-100" aria-label="Close chat">
            <X size={18} />
          </button>
        </div>
      ...
      {!open && (
        <div className="relative flex items-end justify-end">
          {/* Tooltip bubble */}
          {showTooltip && (
            <div className="absolute bottom-16 right-0 w-[260px]">
              <div className="relative rounded-2xl border border-gray-200 bg-white px-4 py-3 shadow-xl">
                <button type="button" onClick={dismissTooltip} className="absolute right-2 top-2 rounded-md p-1 text-gray-500 hover:bg-gray-100" aria-label="Dismiss tip">
                  <X size={14} />
                </button>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="text-sm">
                    ...
                    <div className="mt-0.5 text-gray-600">Want help finding the right service or starting a quote?</div>

                    <div className="mt-3 flex gap-2">
                      <button type="button" onClick={openChat} className="rounded-full bg-gray-900 px-3 py-1.5 text-xs font-semibold text-white hover:bg-gray-800">
                        Open chat
                      </button>
                      <button type="button" onClick={dismissTooltip} className="rounded-full border border-gray-300 bg-white px-3 py-1.5 text-xs font-semibold text-gray-700 hover:bg-gray-50">
                        Not now
                      </button>
                    </div>
                  </div>
          ...
          <button
            ...
            className="group relative inline-flex h-14 w-14 items-center justify-center rounded-full
